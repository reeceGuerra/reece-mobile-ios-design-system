//
//  ReeceTypographySpecTests.swift
//  ReeceDesignSystem
//
...
final class ReeceTypographySpecTests: XCTestCase {
    ...
    func testBasePointSize_UsesDesignScale() {
        ...
        let spec = ReeceTextStyleToken.h4B.spec
    ...
    func testLineHeightMultiple_h4() {
        ...
        let spec = ReeceTextStyleToken.h4M.spec
    ...
    func testLineHeightMultiple_buttons() {
        ...
        let m = Double(ReeceTextStyleToken.buttonM.spec.lineHeightMultiple()!)
        XCTAssertEqual(m, 24.0/16.0, accuracy: 0.0001)

        // buttonS: 14px / 22px ⇒ 1.5714...
        let s = Double(ReeceTextStyleToken.buttonS.spec.lineHeightMultiple()!)
        XCTAssertEqual(s, 22.0/14.0, accuracy: 0.0001)
    }

    func testWithWeightNumberMapping() {
        let base = ReeceTextSpec(designFontSizePx: 16, weight: .regular, slant: .normal, relativeTo: .body, designLineHeightPx: 24, letterSpacingPercent: 0.0)
