...
struct RDSColorHexTests {
    ...
    func rejectsNonHexCharacters() {
        do {
            ...
            default:
                #expect(false, "Expected .invalidScan, got \(err)")
            }
        } catch {
            #expect(false, "Unexpected error type: \(error)")
        }
    ...
    func formatsSwiftUIColorToHEX() {
        ...
        #expect(RDSColorHex.string(from: redColor, includeAlpha: false) == "#FF0000")

        // GREEN
        let greenColor = RDSColorHex.color(from: "#00FF00", colorSpace: .sRGB)!
        #expect(RDSColorHex.string(from: greenColor, includeAlpha: false) == "#00FF00")

        // BLUE
        let blueColor = RDSColorHex.color(from: "#0000FF", colorSpace: .sRGB)!
        #expect(RDSColorHex.string(from: blueColor, includeAlpha: false) == "#0000FF")

        // 50% alpha red
        let halfRed = RDSColorHex.color(from: "#FF000080", colorSpace: .sRGB)!
        #expect(RDSColorHex.string(from: halfRed, includeAlpha: true) == "#FF000080")
    }
    ...
    func roundTrip_RGB() {
        ...
        let hex = RDSColorHex.string(from: c, includeAlpha: false)
        #expect(hex == original)
    }
    ...
    func roundTrip_RGBA() {
        ...
        let hex = RDSColorHex.string(from: c, includeAlpha: true)
        #expect(hex == original)
    }
}
